...
char *DWC_STRDUP(char const *str)
{
	...
	char *new = DWC_ALLOC(len, GFP_ATOMIC);
...
dwc_spinlock_t *DWC_SPINLOCK_ALLOC(void)
{
	...
	sl = DWC_ALLOC(sizeof(*sl), GFP_KERNEL);
	if (!sl) {
...
{
	dwc_timer_t *t = DWC_ALLOC(sizeof(*t), GFP_KERNEL);
	...
	t->t = DWC_ALLOC(sizeof(*t->t), GFP_KERNEL);
	if (!t->t) {
...
{
	dwc_waitq_t *wq = DWC_ALLOC(sizeof(*wq), GFP_KERNEL);
...
{
	dwc_tasklet_t *t = DWC_ALLOC(sizeof(*t), GFP_KERNEL);
...
{
	dwc_workq_t *wq = DWC_ALLOC(sizeof(*wq), GFP_KERNEL);
			...
			char *format, ...)
{
	...
	container = DWC_ALLOC(sizeof(*container), GFP_ATOMIC);
	if (!container) {
