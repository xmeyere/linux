...
ulong elements = 2;
uint irq[4] = {0x6C, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF};
...
static void ReadCoreConfig(hantrodec_t *dev)
{
    ...
    printk("amount of registered cores: %d\n", dev->cores);
	for (c = 0; c < dev->cores; c++) {
		...
		if (IS_G1(dev->hw_id[c])) {
			...
			tmp = (reg & mask);
			if (tmp & (1 << DWL_VP8_E))
				printk("hantrodec: Core[%d] has VP8\n", c);
			if (tmp & (1 << DWL_VP7_E))
				printk("hantrodec: Core[%d] has VP7\n", c);
			if (tmp & (1 << DWL_WEBP_E))
				printk("hantrodec: Core[%d] has WebP\n", c);
			cfg[c] |= tmp ? 1 << DWL_CLIENT_TYPE_VP8_DEC : 0;
			tmp = (reg >> DWL_AVS_E) & 0x01U;
			if (tmp)
				printk("hantrodec: Core[%d] has AVS\n", c);
			cfg[c] |= tmp ? 1 << DWL_CLIENT_TYPE_AVS_DEC : 0;
			tmp = (reg >> DWL_RV_E) & 0x03U;
			if (tmp)
				printk("hantrodec: Core[%d] has RV\n", c);
			cfg[c] |= tmp ? 1 << DWL_CLIENT_TYPE_RV_DEC : 0;
			/* Post-processor configuration */
			reg = ioread32(dev->hwregs[c] + HANTROPP_SYNTH_CFG * 4);
		}
        else if(dev->hw_id[c] == 0x6732)
        {
            printk("handtrodec G2\n");
        }
        else if(dev->hw_id[c] == 0x8001 && dev->hw_id[c] < 0)
        {
           printk("handtrodec other\n");
        }
        /*else {
			reg = ioread32(dev->hwregs[c] + HANTRODEC_SYNTH_CFG_2 * 4);
			tmp = (reg >> DWL_HEVC_E) & 0x3U;
			if (tmp)
				printk("hantrodec: Core[%d] has HEVC\n", c);
			cfg[c] |= tmp ? 1 << DWL_CLIENT_TYPE_HEVC_DEC : 0;
			tmp = (reg >> DWL_VP9_E) & 0x03U;
			if (tmp)
				printk("hantrodec: Core[%d] has VP9\n", c);
		...
		if (tmp)
			printk("hantrodec: Core[%d] has PP\n", c);
...
int hantrodec_init(void)
{
    ...
    if(base_port == 0xffffffff)
    {
        printk("\x016hantrodec: Init multi core[0] at 0x%16lx\n                     core[1] at 0x%16lx\n                     core[2] at 0x%16lx\n                     core[3] at 0x%16lx\n          IRQ _0=%i\n          IRQ_1=%i\n",multicorebase[0],multicorebase[1],multicorebase[2],multicorebase[3],irq[0],irq[1]);
    }
    else
    {
        multicorebase[0] = base_port;
        elements = 1;
        printk("\x016hantrodec: Init single core at 0x%08lx IRQ=%i\n",base_port,irq[0]);
    }

	hantrodec_data.cores = 0;
	hantrodec_data.iosize[0] = 1748;
	hantrodec_data.iosize[1] = 1748;
    ...
    //hantrodec_data.hwregs[0] = (u8*)0x000133f8;
    //hantrodec_data.hwregs[1] = (u8*)0x000133fc;

    writel(readl((void*)0xfe100120) | 2, (void*)0xfe100120); //Turn on the device
	...
	irq_0 = irq[0];//platform_get_irq_byname(pdev, "irq_hantro_g1");
	if (irq_0 > 0) {
...
static int CheckHwId(hantrodec_t *dev)
{
	...
	for (i = 0; i < dev->cores; i++) {
		if (dev->hwregs[i] != NULL) {
			...
			printk("hantrodec: Core %d HW ID=0x%16lx\n", i, hwid);
			hwid = (hwid >> 16) & 0xFFFF; /* product version only */
			while (num_hw--) {
				if (!(( hwid != 0x6731 && hwid != 0x8001 && hwid != 0x6732))) {
					printk("hantrodec: Supported HW found at 0x%16lx\n",
							multicorebase[i]);
					found++;
					dev->hw_id[i] = hwid;
					break;
...
static int ReserveIO(void)
{
	...
	for (i = 0; i < HXDEC_MAX_CORES; i++) {
		if (multicorebase[i] != -1) {
			...
			hantrodec_data.hwregs[i] = (volatile u8 *) ioremap_nocache(multicorebase[i], hantrodec_data.iosize[i]);
            ...
            printk("%x", readl(hantrodec_data.hwregs[i]));
		}
	}
